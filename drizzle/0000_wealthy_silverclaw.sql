...
CREATE TABLE "brainwritings" (
	...
	"is_results_public" boolean DEFAULT false NOT NULL,
	"created_at" timestamp DEFAULT now() NOT NULL,
	"updated_at" timestamp DEFAULT now() NOT NULL,
...
CREATE TABLE "mandalarts" (
	...
	"created_at" timestamp DEFAULT now() NOT NULL,
	"updated_at" timestamp DEFAULT now() NOT NULL,
	CONSTRAINT "mandalarts_public_token_unique" UNIQUE("public_token")
);
--> statement-breakpoint
CREATE TABLE "osborn_ai_generations" (
	"id" serial PRIMARY KEY NOT NULL,
...
CREATE TABLE "osborn_checklists" (
	...
	"created_at" timestamp DEFAULT now() NOT NULL,
	"updated_at" timestamp DEFAULT now() NOT NULL,
	CONSTRAINT "osborn_checklists_public_token_unique" UNIQUE("public_token")
);
--> statement-breakpoint
CREATE TABLE "users" (
...
ALTER TABLE "osborn_ai_generations" ADD CONSTRAINT "osborn_ai_generations_osborn_checklist_id_osborn_checklists_id_fk" FOREIGN KEY ("osborn_checklist_id") REFERENCES "public"."osborn_checklists"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
