...
vi.mock("@/lib/osborn-checklist", () => ({
  updateOsbornChecklistIsResultsPublic: vi.fn(),
}));
...
describe("PATCH /api/osborn-checklists/[id]/results-public", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("認証なしで401を返す", async () => {
    ...
    const request = new NextRequest("http://localhost:3000/api/osborn-checklists/1/results-public", {
      method: "PATCH",
      body: JSON.stringify({
  ...
  it("無効なIDの場合に400を返す", async () => {
    ...
    const request = new NextRequest("http://localhost:3000/api/osborn-checklists/invalid/results-public", {
      method: "PATCH",
      body: JSON.stringify({
        isResultsPublic: true,
      }),
  ...
  it("正常に結果公開状態を更新できる（true）", async () => {
    ...
    vi.mocked(updateOsbornChecklistIsResultsPublic).mockResolvedValue(undefined);
    ...
    expect(updateOsbornChecklistIsResultsPublic).toHaveBeenCalledWith(1, "user-123", true);
  });
  ...
  it("正常に結果公開状態を更新できる（false）", async () => {
    ...
    expect(updateOsbornChecklistIsResultsPublic).toHaveBeenCalledWith(1, "user-123", false);
  });
  ...
  it("サーバーエラーが発生した場合に500を返す", async () => {
    ...
    vi.mocked(updateOsbornChecklistIsResultsPublic).mockRejectedValue(new Error("Database error"));
