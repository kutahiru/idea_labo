...
// ローディングスピナー
export function LoadingSpinner({ className = "h-6 w-6" }: IconProps) {
  return (
    <div className={`animate-spin rounded-full border-b-2 border-blue-500 ${className}`}></div>
  );
}

// 時計アイコン
export function ClockIcon({ className = "h-3 w-3", size }: IconProps) {
  return (
    <svg
      className={className}
      width={size}
